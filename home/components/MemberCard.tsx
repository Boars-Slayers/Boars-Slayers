  ...
  return (
    <>
      ...
      >
        ...
        {/* Badges Preview - More Prominent */}
        {member.badges && member.badges.length > 0 && (
          <div className="flex justify-center flex-wrap gap-2 px-4 pb-4 mt-2">
            {member.badges.slice(0, 5).map((badge) => (
              <button
                key={badge.id}
                onClick={(e) => handleBadgeClick(e, badge)}
                className="relative group/badge transition-transform hover:scale-125 hover:z-10 focus:outline-none"
                title={badge.description}
              >
                <div className="absolute inset-0 bg-gold-500/20 blur-md rounded-full opacity-0 group-hover/badge:opacity-100 transition-opacity"></div>
                <img
                  src={badge.image_url}
                  alt="Badge"
                  className="relative h-8 w-8 rounded border border-gold-600/40 bg-stone-900/80 object-cover shadow-sm group-hover/badge:border-gold-400"
                />
              </button>
            ))}
          </div>
